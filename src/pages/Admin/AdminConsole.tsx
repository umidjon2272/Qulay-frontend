...
import { Activity, ArrowLeft, ArrowUpDown, BarChart3, Bell, Bot, Check, ChevronLeft, ChevronRight, CircleHelp, CreditCard, FileText, Globe2, HeartPulse, LayoutDashboard, LogOut, Menu, Moon, Search, Settings2, Shield, Sun, Users, X, Zap } from "lucide-react";
import type { LucideIcon } from "lucide-react";
import { useAuth } from "../../hooks/useAuth";
import { getApiErrorMessage } from "../../services/api/apiClient";
import { adminApi, type AdminActivity, type AdminConnectionCounts, type AdminFiles, type AdminIntegrations, type AdminNotifications, type AdminOverview, type AdminPlan, type AdminRange, type AdminSubscriptionRequest, type AdminSettingsSection, type AdminSystem, type AdminUsage, type AdminUser, type AdminUserDetail, type NormalizedAdminSettings } from "../../services/api/adminApi";
...
const getNavGroups = (t: TFn): NavGroup[] => [
  { label: t("admin.nav.group.control", "BOSHQARUV"), items: [
    ...
    { label: "Obuna so‘rovlari", path: "/admin/subscriptions", icon: CreditCard },
...
const connectionLabels = (t: TFn): Record<string, string> => ({ connected: t("admin.connection.connected", "Ulangan"), disconnected: t("admin.connection.disconnected", "Ulanmagan"), degraded: t("admin.connection.degraded", "Cheklangan"), authorizing: t("admin.connection.authorizing", "Ulanmoqda"), error: t("admin.connection.error", "Xato") });
...
const formatNumber = (value: number | undefined) => value === undefined ? "—" : new Intl.NumberFormat(adminIntlLocale()).format(value);
...
export const formatLoginDate = (t: TFn, value?: string | null) => value ? new Intl.DateTimeFormat(adminIntlLocale(), { dateStyle: "medium", timeStyle: "short" }).format(new Date(value)) : t("admin.neverLoggedIn", "Hali kirmagan");
export const formatActivityDate = (t: TFn, value?: string | null) => value ? new Intl.DateTimeFormat(adminIntlLocale(), { dateStyle: "medium", timeStyle: "short" }).format(new Date(value)) : t("admin.noActivity", "Faollik mavjud emas");
const formatBytes = (value: number | undefined) => { if (value === undefined) return "—"; const bytes = value; if (bytes < 1024) return `${bytes} B`; const units = ["KB", "MB", "GB", "TB"]; let size = bytes / 1024; let i = 0; while (size >= 1024 && i < units.length - 1) { size /= 1024; i += 1; } return `${size.toFixed(size >= 10 ? 0 : 1)} ${units[i]}`; };
const formatUptime = (t: TFn, totalSeconds: number | undefined) => { if (totalSeconds === undefined) return "—"; const seconds = totalSeconds; const days = Math.floor(seconds / 86400); const hours = Math.floor((seconds % 86400) / 3600); const minutes = Math.floor((seconds % 3600) / 60); if (days > 0) return t("admin.uptime.daysHours", "{days} kun {hours} soat", { days, hours }); if (hours > 0) return t("admin.uptime.hoursMinutes", "{hours} soat {minutes} daqiqa", { hours, minutes }); return t("admin.uptime.minutes", "{minutes} daqiqa", { minutes }); };
...
const AdminOverviewPage = () => {
  ...
  const load = () => { setLoading(true); setData(null); setError(null); void adminApi.overview(range).then(setData).catch((reason: unknown) => setError(getApiErrorMessage(reason))).finally(() => setLoading(false)); };
...
const UsersPage = () => {
  ...
  const load = () => { setLoading(true); setUsers(null); setError(null); void adminApi.users({ page, search, status, role, sort, order }).then(setUsers).catch((reason: unknown) => setError(getApiErrorMessage(reason))).finally(() => setLoading(false)); };
  ...
  return <div className="admin-page admin-fade-in"><PageHeader eyebrow={t("admin.users.eyebrow", "KATALOG")} title={t("admin.nav.users", "Foydalanuvchilar")} description={t("admin.users.description", "Platforma kirish huquqlarini, rollarni va foydalanuvchi faolligini boshqaring.")} /><section className="admin-card">
    ...
    {error ? <ErrorState error={error} retry={load} /> : loading && !users ? <Loading /> : <><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>{thUser}</th><th>{thEmail}</th><th>{thRole}</th><th>{thStatus}</th><th>{thRegistered}</th><th>{thLastActivity}</th><th>{thIntegrations}</th><th /></tr></thead><tbody>{users?.items.map((user) => <tr key={user.id}>
      ...
      <td data-label={thLastActivity}>{formatActivityDate(t, user.lastActivityAt)}</td>
...
const UserDetailPage = ({ id }: { id: string }) => {
  const { t } = useI18n();
  const navigate = useNavigate(); const [user, setUser] = useState<AdminUserDetail | null>(null); const [plans,setPlans]=useState<AdminPlan[]>([]); const [creditAmount,setCreditAmount]=useState("500"); const [loading, setLoading] = useState(true); const [error, setError] = useState<string | null>(null); const [saving, setSaving] = useState(false); const [confirm, setConfirm] = useState<ConfirmState>(null);
  const load = () => { setLoading(true); setUser(null); setError(null); void adminApi.user(id).then(setUser).catch((reason: unknown) => setError(getApiErrorMessage(reason))).finally(() => setLoading(false)); };
  ...
  return <div className="admin-page admin-fade-in">
    ...
    <div className="admin-detail-grid">
      <section className="admin-card admin-profile"><span className="admin-card__eyebrow">{t("admin.profile.eyebrow", "PROFIL")}</span><div className="admin-profile__top"><span className="admin-avatar admin-avatar--large">{initials(user)}</span><div><h2>{user.firstName} {user.lastName}</h2><span>{user.email}</span></div></div><DetailRow label={t("admin.th.role", "Rol")}><span className={`admin-pill admin-pill--${user.role.toLowerCase()}`}>{roleLabels(t)[user.role] ?? user.role}</span></DetailRow><DetailRow label={t("admin.th.status", "Holat")}><span className={`admin-status admin-status--${user.status.toLowerCase()}`}><i />{statusLabels(t)[user.status] ?? user.status}</span></DetailRow><DetailRow label={t("admin.th.registered", "Ro'yxatdan o'tgan")}>{formatDate(t, user.createdAt)}</DetailRow><DetailRow label={t("admin.lastLogin", "Oxirgi kirish")}>{formatLoginDate(t, user.lastLoginAt)}</DetailRow><DetailRow label={t("admin.th.lastActivity", "Oxirgi faollik")}>{formatActivityDate(t, user.lastActivityAt)}</DetailRow></section>
    ...
    <section className="admin-card admin-actions-card"><span className="admin-card__eyebrow">{t("admin.actionsCard.eyebrow", "ADMIN AMALLARI")}</span><CardHeading title={t("admin.actionsCard.title", "Kirish, rol va tarifni boshqarish")} detail={t("admin.actionsCard.detail", "O'zgarishlar real akkauntga qo'llanadi")} />
      <div className="admin-actions-row">
        <div className="admin-actions-row__field"><label>{t("admin.changeRole.label", "Rolni o'zgartirish")}</label><select className="admin-inline-select" value={user.role} disabled={saving} onChange={(event) => askRole(event.target.value as "USER" | "ADMIN")}><option value="USER">{t("admin.role.user", "Foydalanuvchi")}</option><option value="ADMIN">{t("admin.role.admin", "Administrator")}</option></select></div>
        <div className="admin-actions-row__field"><label>{t('admin.plan.assign','Tarifni biriktirish')}</label><select className="admin-inline-select" value={user.subscription?.tier ?? 'STARTER'} disabled={saving} onChange={(event)=>{setSaving(true);void adminApi.assignSubscription(id,event.target.value as AdminPlan['tier']).then(load).catch((reason:unknown)=>setError(getApiErrorMessage(reason))).finally(()=>setSaving(false))}}>{plans.map((plan)=><option key={plan.tier} value={plan.tier}>{plan.name}</option>)}</select></div>
        <div className="admin-actions-row__field"><label>Bonus kredit {user.subscription?.bonusCredits ? `(+${formatNumber(user.subscription.bonusCredits)})` : ''}</label><input className="admin-toolbar-input" type="number" min="1" max="100000" step="1" value={creditAmount} disabled={saving} onChange={(event)=>setCreditAmount(event.target.value)} /></div>
        <button className="admin-button" type="button" disabled={saving || !Number.isInteger(Number(creditAmount)) || Number(creditAmount) < 1 || Number(creditAmount) > 100000} onClick={()=>{const amount=Number(creditAmount);setSaving(true);setError(null);void adminApi.addSubscriptionCredits(id,amount).then(load).catch((reason:unknown)=>setError(getApiErrorMessage(reason))).finally(()=>setSaving(false))}}><Zap size={15}/> Kredit qo‘shish</button>
        {user.status === "BLOCKED" ? <button className="admin-button admin-button--success" disabled={saving} onClick={() => askStatus("ACTIVE")}><Check size={15} /> {t("admin.unblock", "Blokdan chiqarish")}</button> : <button className="admin-button admin-button--danger" disabled={saving} onClick={() => askStatus("BLOCKED")}><X size={15} /> {t("admin.block", "Bloklash")}</button>}
      </div>
      {user.subscription && <div style={{marginTop:14,color:'var(--text-secondary)',fontSize:11}}>Obuna: <strong style={{color:'var(--text-primary)'}}>{plans.find((plan)=>plan.tier===user.subscription?.tier)?.name ?? user.subscription.tier}</strong> · {user.subscription.status}{user.subscription.currentPeriodEnd ? ` · ${new Date(user.subscription.currentPeriodEnd).toLocaleDateString(adminIntlLocale())} gacha` : ''}</div>}
      {user.pendingSubscriptionRequest && <div style={{marginTop:8,color:'var(--text-secondary)',fontSize:11}}>Kutilayotgan so‘rov: <strong style={{color:'var(--text-primary)'}}>{plans.find((plan)=>plan.tier===user.pendingSubscriptionRequest?.tier)?.name ?? user.pendingSubscriptionRequest.tier}</strong></div>}
    </section>
    <div className="admin-detail-grid">
      <section className="admin-card"><span className="admin-card__eyebrow">{t("admin.nav.integrations", "INTEGRATSIYALAR")}</span><CardHeading title={t("admin.connectedServices.title", "Ulangan xizmatlar")} detail={t("admin.metadataOnly", "Faqat metama'lumot")} />{([['telegram','Telegram'],['google','Google'],['bito','Bito ERP'],['whatsapp','WhatsApp'],['instagram','Instagram']] as const).map(([key,label]) => { const connection=user.integrations[key]; return <div className="admin-connection-row" key={key}><span>{label}</span><b className={connection.connected ? "is-connected" : ""}>{connection.productEnabled === false ? t("admin.comingSoon", "Tez orada") : connection.connected ? t("admin.connection.connected", "Ulangan") : connectionLabels(t)[connection.status?.toLowerCase()] ?? connectionLabels(t).disconnected}</b></div>; })}</section>
...
const SubscriptionRequestsPage = () => {
  const [items, setItems] = useState<AdminSubscriptionRequest[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [busy, setBusy] = useState<string | null>(null);
  const load = () => { setLoading(true); setError(null); void adminApi.subscriptionRequests('PENDING').then(setItems).catch((reason: unknown) => setError(getApiErrorMessage(reason))).finally(() => setLoading(false)); };
  useEffect(load, []);
  const review = (id: string, approve: boolean) => {
    if (busy) return;
    setBusy(id);
    setError(null);
    const action = approve ? adminApi.approveSubscriptionRequest(id) : adminApi.rejectSubscriptionRequest(id);
    void action.then(load).catch((reason: unknown) => setError(getApiErrorMessage(reason))).finally(() => setBusy(null));
  };
  return <div className="admin-page admin-fade-in"><PageHeader eyebrow="OBUNA BOSHQARUVI" title="Obuna so‘rovlari" description="User tanlagan tariflarni tekshiring. Tasdiqlanganda obuna 1 oyga faollashadi." />
    {error && <ErrorState error={error} retry={load} />}
    {loading && !items.length ? <Loading /> : <section className="admin-card">{items.length ? <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Foydalanuvchi</th><th>Tarif</th><th>Narx</th><th>So‘rov vaqti</th><th>Joriy holat</th><th /></tr></thead><tbody>{items.map((item) => <tr key={item.id}>
      <td data-label="Foydalanuvchi"><Link className="admin-user-cell" to={`/admin/users/${item.user.id}`}><span className="admin-avatar">{initials(item.user)}</span><span><strong>{item.user.firstName} {item.user.lastName}</strong><small>{item.user.email}</small></span></Link></td>
      <td data-label="Tarif"><strong>{item.plan.name}</strong></td>
      <td data-label="Narx">{new Intl.NumberFormat(adminIntlLocale()).format(item.plan.monthlyPrice)} {item.plan.currency}</td>
      <td data-label="So‘rov vaqti">{formatDate((_, fallback) => fallback, item.requestedAt)}</td>
      <td data-label="Joriy holat">{item.user.subscription?.status === 'ACTIVE' ? `${item.user.subscription.tier} faol` : 'Faol obuna yo‘q'}</td>
      <td data-label=""><div className="admin-actions-row"><button className="admin-button admin-button--success" disabled={busy===item.id} onClick={() => review(item.id,true)}><Check size={15}/> Tasdiqlash</button><button className="admin-button admin-button--danger" disabled={busy===item.id} onClick={() => review(item.id,false)}><X size={15}/> Rad etish</button></div></td>
    </tr>)}</tbody></table></div> : <EmptyState title="Kutilayotgan obuna so‘rovi yo‘q" text="User tarif tanlaganda so‘rov shu yerda paydo bo‘ladi." />}</section>}
  </div>;
};
...
const sectionTitles = (t: TFn): Record<string, string> => ({ overview: t("admin.nav.overview", "Umumiy ko'rinish"), users: t("admin.nav.users", "Foydalanuvchilar"), subscriptions: "Obuna so‘rovlari", user: t("admin.section.userDetail", "Foydalanuvchi tafsiloti"), usage: t("admin.nav.usage", "Foydalanish"), integrations: t("admin.nav.integrations", "Integratsiyalar"), notifications: t("admin.nav.notifications", "Bildirishnomalar"), files: t("admin.nav.files", "Fayllar"), activity: t("admin.nav.activity", "Faoliyat jurnali"), system: t("admin.nav.system", "Tizim holati"), settings: t("admin.nav.settings", "Sozlamalar") });
...
const NotificationsView = ({ data }: { data: AdminNotifications }) => {
  ...
  const successRate = attempted ? Math.round(totalDelivered / attempted * 100) : null;
...
const SystemView = ({ data }: { data: AdminSystem }) => { const { t } = useI18n(); return <div className="admin-grid admin-grid--secondary"><section className="admin-card"><CardHeading title={t("admin.system.uptimeInfoTitle", "Ishlash vaqti ma'lumotlari")} /><div className="admin-health-list">{[["API", healthLabels(t)[data?.api?.status] ?? data?.api?.status, data?.api?.status === "ok"], [t("admin.system.database", "Ma'lumotlar bazasi"), healthLabels(t)[data?.database?.status] ?? data?.database?.status, data?.database?.status === "ok"], [t("admin.system.dbLatency", "Baza kechikishi"), data?.database?.latencyMs === undefined ? "—" : `${formatNumber(data.database.latencyMs)} ms`, data?.database?.status === "ok"], [t("admin.system.notificationWorker", "Bildirishnoma workeri"), healthLabels(t)[data?.notificationWorker?.status] ?? data?.notificationWorker?.status, data?.notificationWorker?.status === "running"], [t("admin.system.environment", "Muhit"), envLabels(t)[data?.environment] ?? data?.environment, Boolean(data?.environment)], [t("admin.system.uptime", "Ishlash vaqti"), formatUptime(t, data?.uptimeSeconds), data?.uptimeSeconds !== undefined], [t("admin.system.migrations", "Migratsiyalar"), data?.migrations?.status === "not_checked" ? t("admin.notChecked", "Tekshirilmagan") : data?.migrations?.status, false]].map(([label, value, healthy]) => <div key={label as string}><span>{label}</span><b className={healthy ? "is-healthy" : ""}>{String(value ?? "—")}</b></div>)}</div></section><section className="admin-card"><CardHeading title={t("admin.system.integrationsTitle", "Integratsiya holati")} detail={t("admin.system.noDataExposed", "Hech qanday ma'lumot ochilmaydi")} /><ConnectionCard label="Telegram" data={data?.integrations?.telegram} icon={<Zap size={21} />} /><ConnectionCard label="Google Calendar" data={data?.integrations?.googleCalendar} icon={<Globe2 size={21} />} /><ConnectionCard label="Google Drive" data={data?.integrations?.googleDrive} icon={<Globe2 size={21} />} /><ConnectionCard label="Bito ERP" data={data?.integrations?.bito} icon={<Zap size={21} />} /></section></div>; };
...
export const SettingsView = ({ data }: { data: NormalizedAdminSettings | null }) => {
  ...
  const [telegramRuntimeBusy, setTelegramRuntimeBusy] = useState(false);
  const [telegramRuntimeMessage, setTelegramRuntimeMessage] = useState<string | null>(null);

  useEffect(() => {
    if (!data || data.missingSections.includes("platform")) return;
    setPlatformName(data.data.platform.name);
  ...
  const inspectTelegramRuntime = async () => {
    if (telegramRuntimeBusy) return;
    setTelegramRuntimeBusy(true); setTelegramRuntimeMessage(null);
    try {
      const result = await adminApi.getTelegramRuntimeDiagnostic();
      const telegram = result.telegram;
      setTelegramRuntimeMessage(`Commit ${result.deploymentVersion} · Node ${result.nodeVersion} (${result.nodeEngine ?? "engine noma’lum"}) · lock ${telegram.lockedVersion ?? "topilmadi"} · installed ${telegram.installedPackageVersion} · GramJS runtime ${telegram.gramJsRuntimeVersion} · lock mosligi: ${telegram.lockMatchesInstalled ? "ha" : "yo‘q"}`);
    } catch (error) {
      setTelegramRuntimeMessage(getApiErrorMessage(error));
    } finally { setTelegramRuntimeBusy(false); }
  };

  return <div className="admin-grid admin-grid--settings">
    <PlanEditor />
    {isMissing("platform") ? <section className="admin-card"><CardHeading title={t("admin.settings.platformTitle", "Platforma sozlamalari")} detail={t("admin.settings.platformDetail", "O'zgarishlar real platformaga qo'llanadi")} /><SettingsMissing /></section> : <section className="admin-card admin-settings-editable"><CardHeading title={t("admin.settings.platformTitle", "Platforma sozlamalari")} detail={t("admin.settings.platformDetail", "O'zgarishlar real platformaga qo'llanadi")} />
    ...
    <SettingsSection title={t("admin.nav.integrations", "Integratsiyalar")} detail={t("admin.settings.integrationsDetail", "Sozlangan/sozlanmagan holat, sirlarsiz")} missing={isMissing("integrations")}>
      ...
      <div className="admin-settings-save-row"><button type="button" className="admin-button admin-button--ghost" disabled={telegramRuntimeBusy} onClick={() => void inspectTelegramRuntime()}>{telegramRuntimeBusy ? "Tekshirilmoqda..." : "Telegram runtime versiyalarini tekshirish"}</button>{telegramRuntimeMessage && <span role="status">{telegramRuntimeMessage}</span>}</div>
      ...
      <SettingsRow label="Bito ERP" value={settings.integrations.bito.configured ? t("admin.configured", "Sozlangan") : t("admin.notConfigured", "Sozlanmagan")} healthy={settings.integrations.bito.configured} />
      <SettingsRow label="WhatsApp" value={t("admin.comingSoon", "Tez orada")} />
      <SettingsRow label="Instagram" value={t("admin.comingSoon", "Tez orada")} />
...
const AdminConsole = () => {
  ...
  const content = section === "overview" ? <AdminOverviewPage /> : section === "users" && !detailMatch ? <UsersPage /> : section === "subscriptions" ? <SubscriptionRequestsPage /> : section === "user" && detailMatch ? <UserDetailPage id={detailMatch[1]} key={detailMatch[1]} /> : section === "activity" ? <ActivityPage key={section} /> : ["usage", "integrations", "notifications", "files", "system", "settings"].includes(section) ? <DataPage kind={section as "usage"} key={section} /> : <AdminOverviewPage />;
