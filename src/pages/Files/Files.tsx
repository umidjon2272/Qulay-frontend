...
const Files = () => {
  ...
  const load = useCallback(async () => {
    ...
    catch (caught) { setError(getApiErrorMessage(caught, t("files.loadError", "Fayllarni yuklashda xatolik yuz berdi."))); }
  ...
  const confirmRemoveCurrentFolder = async () => {
    if (!folderId) return;
    try { await deleteFolder(folderId); setFolderId(undefined); await load(); showToast(t("files.folderDeleted", "Papka o'chirildi"), "success"); }
  ...
  return <main className="files-page">
    ...
    {preview && <div className="file-preview__overlay" onClick={() => setPreview(null)}><div className="file-preview" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><button type="button" onClick={() => setPreview(null)} aria-label={t("files.closePreview", "Previewni yopish")}><X size={16} /></button><h2>{preview.file.originalName}</h2>{preview.url && <img src={preview.url} alt={preview.file.originalName} />}{preview.text !== undefined && <pre>{preview.text}</pre>}<p>{fileType(preview.file)} · {formatSize(preview.file.sizeBytes)}</p></div></div>}
