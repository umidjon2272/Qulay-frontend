...
const Calendar = () => {
  ...
  useEffect(() => { void loadCalendarEvents().catch(() => showToast(t("calendar.loadError", "Uchrashuvlarni yuklab bo'lmadi"), "error")); }, [showToast, t]);
  ...
  const saveMeeting = async () => {
    if (!newTitle.trim()) {
      showToast(t("calendar.enterTitle", "Uchrashuv nomini kiriting"), "error");
      return;
    }
    if (!newDate || !newTime || !newEndTime || savingRef.current) {
      if (!newDate || !newTime || !newEndTime) showToast(t("calendar.selectDateTime", "Sana va vaqtni tanlang"), "error");
      return;
    }
    if (minutes(newEndTime) <= minutes(newTime)) { showToast(t("calendar.endAfterStart", "Tugash vaqti boshlanish vaqtidan keyin bo'lsin"), "error"); return; }
    if (hasTimeConflict()) { showToast(t("calendar.timeConflict", "Bu vaqtda boshqa uchrashuv bor. Vaqtni o'zgartiring."), "error"); return; }
    ...
    try {
      ...
      showToast(saved.googleSyncError ? `${t("calendar.updatedWithSyncError", "Uchrashuv yangilandi, lekin Google sync xatosi:")} ${saved.googleSyncError}` : t("calendar.updated", "Uchrashuv yangilandi"), saved.googleSyncError ? "error" : "success");
    } else {
      const saved = await createMeetingRecord(payload);
      showToast(saved.googleSyncError ? `${t("calendar.createdWithSyncError", "Uchrashuv saqlandi, lekin Google sync xatosi:")} ${saved.googleSyncError}` : t("calendar.created", "Uchrashuv kalendarga qo‘shildi"), saved.googleSyncError ? "error" : "success");
      }
      setEvents(getCalendarEvents());
      closeModal();
    } catch {
      showToast(t("calendar.saveError", "Uchrashuvni saqlab bo'lmadi"), "error");
    } finally {
  ...
  const confirmRemoveMeeting = async () => {
    ...
    try { result = await deleteMeeting(event.id); } catch { showToast(t("calendar.deleteError", "Uchrashuvni o'chirib bo'lmadi"), "error"); return; }
    setEvents(getCalendarEvents());
    setPendingDelete(null);
    showToast(result.googleSync.synced ? t("calendar.deleted", "Uchrashuv o‘chirildi") : `${t("calendar.deletedWithSyncError", "Uchrashuv o‘chirildi, lekin Google sync xatosi:")} ${result.googleSync.errorCode}`, result.googleSync.synced ? "success" : "error");
  };
  ...
  return (
    <main className="calendar-page">
      <header className="calendar-page__header">
        <div>
          <span className="calendar-page__eyebrow">{t("calendar.pageEyebrow", "YOUR SCHEDULE")}</span>
      ...
      <div className="calendar-layout">
        <section className="calendar-card">
          <div className="calendar-card__top">
            <div className="calendar-card__month">
              <button type="button" onClick={() => setMonth(-1)} aria-label={t("calendar.prevMonth", "Oldingi oy")}><ChevronLeft size={17} /></button>
              <h2>{monthNames[month]} {year}</h2>
              <button type="button" onClick={() => setMonth(1)} aria-label={t("calendar.nextMonth", "Keyingi oy")}><ChevronRight size={17} /></button>
            </div>
            <div className="calendar-view-switch" role="group" aria-label={t("calendar.viewSwitch", "Kalendar ko'rinishi")}><button type="button" className={view === "month" ? "is-active" : ""} onClick={() => setView("month")}>{t("calendar.month", "Oy")}</button><button type="button" className={view === "week" ? "is-active" : ""} onClick={() => setView("week")}>{t("calendar.week", "Hafta")}</button><button type="button" className={view === "day" ? "is-active" : ""} onClick={() => setView("day")}>{t("calendar.day", "Kun")}</button></div>
        ...
        <aside className="calendar-sidebar">
          <div className="calendar-sidebar__header">
            <div><span>{t("calendar.selectedDay", "SELECTED DAY")}</span><h2>{selectedDay} {monthNames[month]}</h2></div>
          ...
          <div className="calendar-events">
            {selectedEvents.map((event) => (
              <article className={`calendar-event calendar-event--${event.type}`} key={event.id}>
                ...
                <div className="calendar-event__body">
                  ...
                  {event.reminder && <small>{t("calendar.reminder", "Eslatma")}: {event.reminder}</small>}
                </div>
                <div className="calendar-event__actions">
                  <button type="button" onClick={() => openEditModal(event)} aria-label={t("calendar.editAria", "Uchrashuvni tahrirlash")}><Pencil size={12} /></button>
                  <button type="button" onClick={() => removeMeeting(event)} aria-label={t("calendar.deleteAria", "Uchrashuvni o‘chirish")}><Trash2 size={12} /></button>
            ...
            {selectedEvents.length === 0 && (
              <div className="calendar-events__empty">
                ...
                <strong>{t("calendar.noEventsTitle", "Event yo‘q")}</strong>
                <span>{t("calendar.emptyDay", "Tanlangan kun uchun hali uchrashuv belgilanmagan.")}</span>
              </div>
            )}
          </div>
          <button type="button" className="calendar-sidebar__button" onClick={openCreateModal}><Plus size={14} />{t("calendar.addEvent", "Event qo‘shish")}</button>
        </aside>
      </div>

      <section className="calendar-summary">
        <div className="calendar-summary__icon"><Clock3 size={17} /></div>
        <div><span>{t("calendar.todayPlan", "BUGUNGI REJA")}</span><h2>{t("calendar.todayCount", "Sizda bugun {count} ta reja bor", { count: todayEvents.length })}</h2><p>{nextEvent ? t("calendar.nextMeetingAt", "Keyingi uchrashuv {time} da.", { time: nextEvent.time }) : t("calendar.noMeetingsToday", "Bugun uchun uchrashuv belgilanmagan.")}</p></div>
      </section>

      {showModal && (
        <div className="calendar-modal__overlay" onClick={closeModal}>
          <div className="calendar-modal" onClick={(event) => event.stopPropagation()}>
            <button type="button" className="calendar-modal__close" onClick={closeModal} aria-label={t("calendar.closeModal", "Uchrashuv oynasini yopish")}><X size={17} /></button>
            <div className="calendar-modal__icon"><CalendarDays size={21} /></div>
            <span className="calendar-modal__eyebrow">{editId !== null ? t("calendar.editEventEyebrow", "EDIT EVENT") : t("calendar.newEventEyebrow", "NEW EVENT")}</span>
            <h2>{editId !== null ? t("calendar.edit", "Uchrashuvni tahrirlash") : t("calendar.new", "Yangi uchrashuv")}</h2>
            <p>{t("calendar.enterEventInfo", "Event ma’lumotlarini kiriting.")}</p>
            <input type="text" placeholder={t("calendar.name", "Uchrashuv nomi")} aria-label={t("calendar.name", "Uchrashuv nomi")} value={newTitle} onChange={(event) => setNewTitle(event.target.value)} />
            <input type="date" aria-label={t("calendar.dateAria", "Uchrashuv sanasi")} value={newDate} onChange={(event) => selectDate(event.target.value)} />
            <div className="calendar-modal__row"><label>{t("calendar.start", "Boshlanish")}<input type="time" aria-label={t("calendar.startTimeAria", "Boshlanish vaqti")} value={newTime} onChange={(event) => setNewTime(event.target.value)} /></label><label>{t("calendar.end", "Tugash")}<input type="time" aria-label={t("calendar.endTimeAria", "Tugash vaqti")} value={newEndTime} onChange={(event) => setNewEndTime(event.target.value)} /></label></div>
            <input type="text" placeholder={t("calendar.participant", "Ishtirokchi")} aria-label={t("calendar.participant", "Ishtirokchi")} value={newParticipant} onChange={(event) => setNewParticipant(event.target.value)} />
            <input type="text" placeholder={t("calendar.location", "Joy yoki manzil")} aria-label={t("calendar.location", "Joy yoki manzil")} value={newLocation} onChange={(event) => setNewLocation(event.target.value)} />
            <textarea rows={3} placeholder={t("calendar.description", "Tavsif")} aria-label={t("calendar.descriptionAria", "Uchrashuv tavsifi")} value={newDescription} onChange={(event) => setNewDescription(event.target.value)} />
            <select aria-label={t("calendar.reminderAria", "Uchrashuv eslatmasi")} value={newReminder} onChange={(event) => setNewReminder(event.target.value)}><option value="15 daqiqa oldin">{t("calendar.reminder15", "15 daqiqa oldin")}</option><option value="1 soat oldin">{t("calendar.reminder1h", "1 soat oldin")}</option><option value="">{t("calendar.noReminder", "Eslatmasin")}</option></select>
      ...
      {pendingDelete && (
        <ConfirmDialog
          title={t("calendar.deleteTitle", "Uchrashuvni o'chirish")}
          description={t("calendar.deleteDescription", "\"{title}\" uchrashuvini o'chirishni tasdiqlaysizmi?", { title: pendingDelete.title })}
