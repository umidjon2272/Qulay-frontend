...
const Memory = () => {
  ...
  const load = useCallback(async () => { try { setItems((await memoryApi.list(search.trim())).items); } catch (error) { showToast(getApiErrorMessage(error, t('memory.loadError', 'AI xotirasini yuklab bo‘lmadi.')), 'error'); } }, [search, showToast, t]);
  ...
  const toggleMemory = async () => { const next=!enabled; setEnabled(next); try { await memoryApi.setPreference(next); } catch { setEnabled(!next); showToast(t('memory.toggleError', 'Xotira sozlamasini saqlab bo‘lmadi'),'error'); } };
  ...
  const removeAll = async () => { if (!window.confirm(t('memory.confirmDeleteAll', 'DIQQAT: barcha AI xotirasi butunlay o‘chiriladi. Davom etasizmi?'))) return; await memoryApi.removeAll(); await load(); showToast(t('memory.allDeleted', 'Barcha xotira o‘chirildi'),'success'); };
  ...
  return <main className="business-page">
    ...
    <div className="business-toolbar"><label className="business-search"><Search size={16}/><input value={search} onChange={(event)=>setSearch(event.target.value)} placeholder={t('memory.searchPlaceholder', 'Xotiradan qidirish...')}/></label><span className="business-badge">{items.length} {t('memory.countSuffix', 'ta faol xotira')}</span></div>
